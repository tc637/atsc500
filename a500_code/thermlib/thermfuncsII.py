...
import textwrap
import sys
from a500_utils.helpers import test_scalar, make_tuple
...
def t_uos_thetal(thetal,qt,p):
    ...
    rs1=rs_tp(t1,p)
    if   (qt>rs1):
        result["temp"]=t1; result["ql"]=qt-rs1; result["issat"]=True;result["qv"]=rs1
    else:
        result["temp"]=t1; result["ql"]=0.0;    result["issat"]=False;result["qv"]=qt
...
def thetae(tds, ts, ps):
    ...
    """
    plcl = LCL(tds, ts, ps)
    thetaLcl = theta(ts, ps)
    Tlcl = tda(thetaLcl, plcl)
    thetae = theta(ts, ps)*np.exp(2.6518986*ws(tds, ps)*1000./Tlcl)
    return thetae
...
def calc_ABcoeffs(thetal0,qt,press):
    ...
    if invert.issat:
        ...
        theta0= theta(invert.temp,press)
...
def testit():
    ...
    np.testing.assert_almost_equal(ws(Td,p),0.00509417)
